...
io.on('connection', (socket) => {
	socket.on('join', (data) => {
		...
		if (!Object.keys(allsockets).includes(room)) {
			...
			socket.emit('message', {
				name: 'server',
				message: `Welcome to Socket-Chat! You are currently in room "${socket.proto.room}". To join a different room, type /join [room]. Type /help to see more commands`
			});
			...
			if (message[0] === '/') {
				...
				switch (message.split(' ')[0]) {
					...
					default:
						socket.emit('message', {
							name: 'server',
							message: 'Error: This command is Unknown if you think this is an error send us a ticket'
						});
				}
			} else {
				toRoom(socket.proto.room).emit('message', {
